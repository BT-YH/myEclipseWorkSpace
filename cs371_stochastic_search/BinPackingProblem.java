...
public class BinPackingProblem implements State {
	...
	public BinPackingProblem(int min, int max, int items, int bins) {
		...
		for (int i = 0; i < items; i++) {
			sizes[i] = random.nextInt(max - min + 1) + min; // assigning weight
			placements[i] = random.nextInt(bins); // initializing placement
			binLoads[placements[i]] += sizes[i]; // placing item in the bins
		}
	...
	public void step() {
		prevItem = random.nextInt(sizes.length);
		prevPlacement = placements[prevItem];
		while (placements[prevItem] == prevPlacement)
			placements[prevItem] = random.nextInt(binLoads.length);
		binLoads[prevPlacement] -= sizes[prevItem];
		binLoads[placements[prevItem]] += sizes[prevItem];
	}

	public void undo() {
		binLoads[placements[prevItem]] -= sizes[prevItem];
		binLoads[prevPlacement] += sizes[prevItem];
		placements[prevItem] = prevPlacement;
	}
