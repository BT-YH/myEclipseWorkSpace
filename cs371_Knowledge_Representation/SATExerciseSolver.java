...
public class SATExerciseSolver {
	...
	public SATSolver get2Solver() {
        ...
        int[][] clauses = {{-A,C},{-B,-C},{B,C},{-A,B,-C},{A,C},{-B,C}}; 
	...
	public SATSolver get5Solver() {
        ...
        SATSolver solver = new SAT4JSolver();
        int[][] clauses = {{-A, B}, {-A, C}, {-A, D}, {-A, E}, {-A, F}, {A, -B, -C, -D, -E, -F},
        		{-B, -C}, {-B, -D}, {-B, -E}, {-B, -F}, {B, C, D, E, F},
        		{-A, -B, C}, {A, -C}, {B, -C},
        		{-A, -B, -D}, {-A, B, C, D}, {A, -B, C, D}, {A, B, -D}, {-C, -D},
        		{-A, -E}, {A, B, C, D, E}, {-B, -E}, {-C, -E}, {-D, -E},
        		{-A, -F}, {A, B, C, D, E, F}, {-B, -F}, {-C, -F}, {-D, -F}, {-E, -F}};
        for (int[] clause : clauses)
            solver.addClause(clause);
        return solver;
	}
	...
	/**
	 * Return a SATSolver loaded with a knowledge base containing the CNF clauses for Final Problem 4. 
	 * For variable numbering, let A = 1, B = 2, C = 3, and D = 4.
	 * @return a SATSolver loaded with a knowledge base containing the CNF clauses for final
	 */
	public SATSolver getFinalSolver() {
		SATSolver solver = new SAT4JSolver();
        final int A = 1, B = 2, C = 3, D = 4;
        int[][] clauses = { {-A, -D}, {A, D}, {-A, -C}, {-B, -C}, {A, B, C}, {-C, -D}, {A, C, D} }; 
        for (int[] clause : clauses)
        	solver.addClause(clause);
        return solver;
	}
...
//		testLiterals(solver);
		...
		SATSolver solver = getFinalSolver();
		showClauses(solver);
		testLiterals(solver);
